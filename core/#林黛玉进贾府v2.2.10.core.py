...
def challenge(int_num, str_num, gw_name, gw_health, gw_attack, gw_defence, gw_combat_effectiveness, plus_gold_coins,
              plus_attack, plus_defence, plus_health, use_invincible_potion, explain='', treasure=''):
...
# 删除存档
def delete_saveload():
    os.remove('C:\ProgramData\DaiyuLinGototheJiaHome\save.json')
    ...
    while True:
        ...
        while x == '1':
            ...
            while l == '1':
                ...
                print('目前解锁关卡为: {}/20\n'
                      '请输入你要挑战的关卡\n'
                      '[+]进入{}\n'
                      '[0]返回首页'.format(now_int_challenge, Saodang_Str_Op))
                print('=' * 39)
                # 关卡判定
                p = clear_input()
                try:
            ...
            while l == '2':
                ...
                print('已解锁英雄副本: {}/2\n'
                      '请输入你要挑战的关卡\n'
                      '[+]进入{}\n'
                      '[0]返回首页'.format((load_data['int_challenge'] - 1) // 10, Saodang_Str_Op))
                print('=' * 39)
                p = clear_input()
                try:
                    p = int(p)
                    if p > ((load_data['int_challenge'] - 1) // 10) - int(Saodang) or p > 2 or p < 0:
                        print('=' * 39)
                        print('你输入的关卡未解锁')
                        print('=' * 39)
                        sleep(1)
        ...
        while x == '3':
            ...
            # 无敌药水
                ...
                load_data = goods('无敌药水', 1500, load_data, '属性:只要你的无敌药水够多没有你通不过的关卡',
                                  '目前无敌药水: %d瓶' % load_data['invincible_potion'], potion=1)
            # 飞升药水
            elif f == '2':
                print('=' * 25)
                show(load_data['gold_coins'], '金币')
                ...
                g = clear_input()
                if g == '':
                    if load_data['health'] < 8000000 and load_data['attack'] < 3000000 and load_data[
                        'defence'] < 2500000:
            ...
            # 属性加强药水
                ...
                load_data = goods('属性加强药水', 2000, load_data, '属性:增加生命1000/攻击300/防御200',
                                  '目前无敌药水: %d瓶' % load_data['invincible_potion'], hp=1000, att=300, df=200)
            # 查询无果面板
            else:
                continue
        # 小游戏面板循环
        while x == '4':
            ...
            if t == '1':
                ...
                pass
        # 充值面板循环
        while x == '5' and load_data['set']['1'] == 1:
            print('=' * 41)
        ...
        # 设置面板循环
        while x == '*':
            char_recharge = '禁用' if load_data['set']['1'] == 1 else '启用'
            print('=' * 59)
                ...
                while True:
                    ...
                    if i == '1':
                        if load_data['set']['2'] == 0:
                            music = os.path.exists('C:\ProgramData\DaiyuLinGototheJiaHome\\music.mp3')
                            if music:
                                if isPause:
                                    mixer.music.unpause()
                                    load_data['set']['2'] = 1
