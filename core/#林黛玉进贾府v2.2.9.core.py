...
def show(name, title):
    ...
    if 10000 <= fabs(name) < 100000000:
        print('%s: %d万' % (title, name // 10000))
    if 100000000 <= fabs(name):
        ...
        while True:
            try:
                ...
                load_data = {'attack': float(input('攻击力：')), 'defence': float(input('防御力：')),
                             ...
                             'int_challenge': int(input('关卡：')), 'gold_coins': float(input('金币：')),
                             'invincible_potion': int(input('无敌药水：')),
                             'name': input('角色名：'), 'set': {'1': 1, '2': 0}}
...
if is_new():
    ...
    load_data = {'attack': 50, 'defence': 20, 'health': 200, 'int_challenge': 1, 'gold_coins': 0,
                 'invincible_potion': 0, 'set': {'1': 0, '2': 0}, 'name': clear_input()}
    while ' ' in load_data['name']:
        print('角色名不能包含空格！')
        print('请输入你的角色名:')
        load_data['name'] = clear_input()
